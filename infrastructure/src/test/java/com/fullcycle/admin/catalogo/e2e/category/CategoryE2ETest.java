...
public class CategoryE2ETest implements MockDsl {
    ...
    @Test
    void asACatalogAdminIShouldBeAbleToSortAllCategoriesByDesc() throws Exception {
        Assertions.assertTrue(MYSQL_CONTAINER.isRunning());
        Assertions.assertEquals(0, categoryRepository.count());
    ...
    @Test
    void asACatalogAdminIShouldNotSeeAnErrorByDeletingNotExistentCategory() throws Exception {
        Assertions.assertTrue(MYSQL_CONTAINER.isRunning());
        Assertions.assertEquals(0, categoryRepository.count());
